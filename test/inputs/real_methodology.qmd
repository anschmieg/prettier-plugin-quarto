...
As is typical in social science, the main challenge is distinguishing _causation_ from _correlation_.
...
The _pooled treatment indicator_ is:
...
The data records _age at modification_, not _calendar year_.
To make this compatible with the LISS panel data, we convert as follows, which gives us the _calendar year of first body modification_.
...
We therefore use _log monthly income_ as the outcome variable:
...
By observing people _before_ and _after_ they get a modification, we can difference out time-invariant confounders,
...
| Coefficient   | Meaning                                                                                                                      |
| ------------- | ---------------------------------------------------------------------------------------------------------------------------- |
| $\beta_0$     | Mean outcome for control group, pre-period; _average income of untreated individuals at the start of the observation window_ |
| $\beta_1$     | Pre-existing difference (selection effect); _structural difference between treated and untreated individuals_                |
| $\beta_2$     | Time trend affecting both groups                                                                                             |
| **$\beta_3$** | The DiD effect; _change in income for treated individuals excluding the common trend_                                        |
...
\mathbb{E}[Y_{i,t+1}(0) - Y_{it}(0) | D_i = 1] = \mathbb{E}[Y_{i,t+1}(0) - Y_{it}(0) | D_i = 0]
